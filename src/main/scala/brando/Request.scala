...
object Request {  
  def apply(command: RedisCommand, params: String*) =
    new Request(command, params map (ByteString(_)): _*)
}

//Helps creating a request like HMSET key k1 v1 k2 v2... 
object HashRequest {
  def apply(cmd: RedisCommand, key: String, map: Map[String, String]) = {
...
object ShardRequest {
  def apply(command: RedisCommand, key: String, params: String*) = {
    new ShardRequest(command, ByteString(key), params map (ByteString(_)): _*)
  }
}
