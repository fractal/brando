...
private class Connection extends Actor with ReplyParser with ActorLogging {
  ...
  def receive = {
    ...
    case x ⇒ log.warning("connection didn't expect - " + x)
  }
}
...
class Brando(
    ...
    port: Int,
    database: Option[Int],
    auth: Option[String]) extends Actor with ActorLogging {
  ...
  def receive = {
    ...
    case x ⇒ log.warning("Unexpected " + x + "\r\n")
  }
}
